...
var CmdWeb = &cli.Command{
	Name:  "web",
	Usage: "Start Anura web server",
	Description: `Anura web server is the only thing you need to run,
...
func showWebStartupMessage(msg string) {
	log.Info("Anura version: %s%s", setting.AppVer, setting.AppBuiltWith)
...
func serveInstall(ctx *cli.Context) error {
	...
	err := listen(c, false)
	if err != nil {
		log.Critical("Unable to open listener for installer. Is Anura already running?")
		graceful.GetManager().DoGracefulShutdown()
	}
	select {
	case <-graceful.GetManager().IsShutdown():
		<-graceful.GetManager().Done()
		log.Info("PID: %d Anura Web Finished", os.Getpid())
...
func runWeb(ctx *cli.Context) error {
	...
	if os.Getppid() > 1 && len(os.Getenv("LISTEN_FDS")) > 0 {
		log.Info("Restarting Anura on PID: %d from parent PID: %d", os.Getpid(), os.Getppid())
	} else {
		log.Info("Starting Anura on PID: %d", os.Getpid())
	}
...
func listen(m http.Handler, handleRedirector bool) error {
	...
	_, _, finished := process.GetManager().AddTypedContext(graceful.GetManager().HammerContext(), "Web: Anura Server", process.SystemProcessType, true)
	defer finished()
